package io.github.bhuwanupadhyay.rtms.customer

import io.github.bhuwanupadhyay.rtms.customer.domain.model.valueobjects.Actions
import io.github.bhuwanupadhyay.rtms.customer.interfaces.rest.dto.CreateAppResource
import io.github.bhuwanupadhyay.rtms.customer.interfaces.rest.dto.ReleaseVersionResource
...
internal class IntegrationTest {
    ...
    fun `return 201 if app created successfully`() {
        ...
        body.jsonPath("$.appId").value { appId: String ->
            body
                    ...
                    .jsonPath("$._links[0].path").isEqualTo("/customers/$appId")
        }
    }
    ...
    fun `return 200 if list by default pagination successfully`() {
        ...
        client
                .get()
                .uri("/customers?number={number}&size={size}", -1, 0).exchange().expectStatus().isOk.expectBody()
    ...
    fun `return 200 if paginate by page number and successfully`() {
        ...
        client
                .get()
                .uri("/customers?number={number}&size={size}", pageNumber, pageSize).exchange().expectStatus().isOk.expectBody()
