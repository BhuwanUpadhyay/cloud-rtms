...
import io.github.bhuwanupadhyay.rtms.payment.domain.model.valueobjects.Actions
import io.github.bhuwanupadhyay.rtms.payment.interfaces.rest.dto.CreateAppResource
import io.github.bhuwanupadhyay.rtms.payment.interfaces.rest.dto.ReleaseVersionResource
import io.github.bhuwanupadhyay.rtms.payment.interfaces.rest.dto.WorkflowResource
...
internal class RtmsAppIntegrationTest {
    ...
    fun `return 201 if app created successfully`() {
        ...
        body.jsonPath("$.appId").value { appId: String ->
            body
                    ...
                    .jsonPath("$._links[0].path").isEqualTo("/payments/$appId")
        }
    }
    ...
    fun `return 200 if list by default pagination successfully`() {
        ...
        client
                .get()
                .uri("/payments?number={number}&size={size}", -1, 0).exchange().expectStatus().isOk.expectBody()
    ...
    fun `return 200 if paginate by page number and successfully`() {
        ...
        client
                .get()
                .uri("/payments?number={number}&size={size}", pageNumber, pageSize).exchange().expectStatus().isOk.expectBody()
