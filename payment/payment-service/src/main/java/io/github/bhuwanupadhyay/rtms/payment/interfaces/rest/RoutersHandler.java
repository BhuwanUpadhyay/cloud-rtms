package io.github.bhuwanupadhyay.rtms.payment.interfaces.rest;

import static org.springframework.web.reactive.function.BodyInserters.fromValue;

import io.github.bhuwanupadhyay.rtms.payment.application.commandservices.AppCommandService;
...
import io.github.bhuwanupadhyay.rtms.payment.infrastructure.repositories.jpa.AppQueryRepository;
import io.github.bhuwanupadhyay.rtms.payment.interfaces.rest.dto.*;
...
public class RoutersHandler {
  ...
  private Link linkOfGet(AppId id) {
    return Link.builder().rel("get").method("GET").path("/payments/" + id.getAppId()).build();
  }

  private Link linkOfCreate() {
    return Link.builder().rel("create").method("POST").path("/payments").build();
  }

  private Link linkOfList() {
    return Link.builder().rel("list").method("GET").path("/payments").build();
  }
        ...
        .map(
            action ->
                Link.builder()
                    .rel(action)
                    .method("PUT")
                    .path("/payments/" + id.getAppId() + "/" + action)
                    .build())
        .collect(Collectors.toList());
  }
